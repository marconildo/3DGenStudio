...
def convert_images_to_video(image_dir, video_path, fps=20, crf=18, preset="slow"):
    ...
    cmd = [
        ...
        "-y",
        "-framerate", str(fps),
        # Windows ffmpeg builds have no glob support in libavformat; use the
        # numeric sequence pattern instead (frames are written as %05d).
        *(["-pattern_type", "glob", "-i", pattern] if os.name != "nt"
          else ["-start_number", "0",
                "-i", os.path.join(os.path.dirname(pattern),
                                   "%05d" + os.path.splitext(pattern)[1])]),
        "-c:v", "libx264",
        "-crf", str(crf),
        "-preset", preset,
        "-pix_fmt", "yuv420p",
