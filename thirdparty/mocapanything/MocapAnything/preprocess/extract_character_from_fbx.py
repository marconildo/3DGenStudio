...
# The 0.01 default encodes an assumption that source rigs are authored in
# CENTIMETRES (Truebones: 126cm eagle). A metre-native source (e.g. exported
# from glTF) needs 1.0, or every motion ends up 100x too small.
...
def extract_character_data(fbx_file_path, save_path, bvh_joint_names=None):
    ...
    bpy.ops.wm.obj_export(
        ...
        export_uv=True,
        export_normals=True,
        # wm.obj_export defaults to up_axis='Y'/forward_axis='NEGATIVE_Z', which
        # rotates the mesh 90deg about X relative to the skeleton BVH written
        # beside it (Blender-native Z-up). Export unconverted so they agree.
        forward_axis='Y',
        up_axis='Z',
    )
